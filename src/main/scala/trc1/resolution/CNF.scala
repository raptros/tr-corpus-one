...
class CNF(var clauses:Map[Int, Clause]) {
  ...
  def uniqueUnificationMatch(literal:Literal):Option[(Int,Literal,Substitution)] = {
    val matchingLiterals : List[(Int,Literal,Substitution)] = for {
    ...
    if (matchingLiterals.size == 1) matchingLiterals.headOption else None
  }
  ...
  /** clauseIx is a singleton clause that has just been involved in a resolution step, so this clause disappears. Remove it from the
    ...
    */
  def resolveAsSingleton(clauseIx:Int, substitution:Substitution):CNF = {
    ...
    applySubstitution2(substitution) { clauses - clauseIx }
  }
  ...
  /** clauseIx is a clause that has just been involved in a resolution step with a singleton targeting the literal 'literal' occurring in
    ...
    */
  def resolveWithSingleton(clauseIx:Int, literal:Literal, substitution:Substitution):CNF = { 
    if (!(clauses contains clauseIx)) {
      throw new Exception("Shouldn't be here: trying to resolve nonexisting clause as singleton " + clauseIx.toString)
    }
    if (!(clauses(clauseIx).literals contains literal)) {
      throw new Exception("Shouldn't be here: trying to resolve with nonexisting literal " + literal.toString)
    }
    applySubstitution2(substitution) { 
