...
trait BaseApplicationExpression[T <: BaseExpression[T]] extends BaseExpression[T] {
   ...
   */
  override def isAtom() =
    this.pred.isInstanceOf[BaseVariableExpression[_]]

  override def constants(): Set[Variable] = {
    val function_constants = this.function match {
      case v: BaseVariableExpression[_] => Set()
  ...
  override def predicates(): Set[Variable] = {
    val function_preds = this.function match {
      case v: BaseVariableExpression[_] => {
  ...
  protected def shouldParenthesizeFunction(function: T): Boolean = {
    if (function.isInstanceOf[BaseLambdaExpression[_]]) {
      val term = function.asInstanceOf[BaseLambdaExpression[_]].term
      if (term.isInstanceOf[BaseApplicationExpression[_]]) {
        if (!term.asInstanceOf[BaseApplicationExpression[_]].function.isInstanceOf[BaseVariableExpression[_]]) {
          return true
        }
      } else if (!function.asInstanceOf[BaseLambdaExpression[_]].term.isInstanceOf[BaseBinaryExpression[_]]) {
        return true
      }
    } else if (function.isInstanceOf[BaseApplicationExpression[_]]) {
      return true
    }
    return false
  }
